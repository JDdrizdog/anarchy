...
namespace Anarchy.Helpers.Helper
{
    public static class IconInjector
    {
        ...
        private class IconFile
        {
            ...
            public int ImageCount => this.iconDir.Count;
            ...
            public static IconFile FromFile(string filename)
            {
                ...
                int num;
                num = Marshal.SizeOf(iconFile.iconDir);
                Type typeFromHandle;
                typeFromHandle = typeof(ICONDIRENTRY);
                int num2;
                num2 = Marshal.SizeOf(typeFromHandle);
                for (int i = 0; i <= iconFile.iconDir.Count - 1; i++)
                {
                    ICONDIRENTRY iCONDIRENTRY;
                    iCONDIRENTRY = (ICONDIRENTRY)Marshal.PtrToStructure(new IntPtr(gCHandle.AddrOfPinnedObject().ToInt64() + num), typeFromHandle);
            ...
            {
                byte[] array;
                array = new byte[Marshal.SizeOf(typeof(ICONDIR)) + Marshal.SizeOf(typeof(GRPICONDIRENTRY)) * this.ImageCount];
                GCHandle gCHandle;
                gCHandle = GCHandle.Alloc(array, GCHandleType.Pinned);
                Marshal.StructureToPtr(this.iconDir, gCHandle.AddrOfPinnedObject(), fDeleteOld: false);
                int num;
                num = Marshal.SizeOf(this.iconDir);
                for (int i = 0; i <= this.ImageCount - 1; i++)
                {
                    GRPICONDIRENTRY structure;
                    structure = default(GRPICONDIRENTRY);
                    BITMAPINFOHEADER bITMAPINFOHEADER;
                    bITMAPINFOHEADER = default(BITMAPINFOHEADER);
                    GCHandle gCHandle2;
                    gCHandle2 = GCHandle.Alloc(bITMAPINFOHEADER, GCHandleType.Pinned);
                    Marshal.Copy(this.ImageData(i), 0, gCHandle2.AddrOfPinnedObject(), Marshal.SizeOf(typeof(BITMAPINFOHEADER)));
                    gCHandle2.Free();
        ...
        {
            IconFile iconFile;
            iconFile = IconFile.FromFile(iconFileName);
            IntPtr hUpdate;
            hUpdate = NativeMethods.BeginUpdateResource(exeFileName, deleteExistingResources: false);
            byte[] array;
            array = iconFile.CreateIconGroupData(iconBaseID);
